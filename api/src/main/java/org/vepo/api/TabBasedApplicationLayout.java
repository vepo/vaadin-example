...
public class TabBasedApplicationLayout extends Composite implements ApplicationLayout {
    ...
    public TabBasedApplicationLayout(String caption) {
        System.out.println("Creating tab based app: " + caption);
    ...
    public Collection<Component> getHeaderComponents() {
        return IntStream.range(0, header.getComponentCount()).mapToObj(header::getComponent)
    ...
    public void closeTab(Component tabContent) {
        List<WorkingAreaComponent> toRemove = workingAreaComponents.stream()
                .filter(component -> component.getComponent().equals(tabContent)).collect(Collectors.toList());
    ...
    public void showComponent(String caption) {
        IntStream.range(0, tabSheet.getComponentCount()).mapToObj(tabSheet::getTab)
                .filter(tab -> tab.getCaption().equals(caption)).forEach(tabSheet::setSelectedTab);
    }
    ...
    private void updateMenuButtonsStyle(String styleName, BiConsumer<Component, String> setOrAddStyleMethod) {
        IntStream.range(0, menuLayout.getComponentCount()).mapToObj(menuLayout::getComponent)
